...
namespace com::github::doevelopper::atlas::launcher::args
{
    ...
    public:
        ...
        //
        // // Access to all options (for more complex scenarios)
        // virtual const std::map<std::string, std::any>& getAllOptions() const = 0;
        //
        // // Method to get unregistered options (options passed on the command line but not defined)
        // virtual const std::vector<std::string>& getUnregisteredOptions() const = 0;
        //
        // // Method to get positional options (options passed without a name)
        // virtual const std::vector<std::string>& getPositionalOptions() const = 0;
    protected:
    private:
    };
}
